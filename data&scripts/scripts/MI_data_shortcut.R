...
#  Load files
#  WQP
allMI.do = read.csv("./Michigan.Data/Profiles/MIDataRetrievalCombined.csv",
...
MI.ALL.Data <- setDT(MI.ALL.Data)

MI.ALL.Data[MI.allsites, on = .(MonitoringLocationIdentifier = MonitoringLocationIdentifier) ,  MonitoringLocationName := MonitoringLocationName  ]
MI.ALL.Data[ , profile_ident := .GRP   , .(MonitoringLocationIdentifier, Year, DOY)]

Link <- setDT(Link)

MI.ALL.Data[Link, on = .(MonitoringLocationIdentifier = wqp_monitoringlocationidentifier) , ':=' ("link_nhd_id" = lake_nhdid, "link_lagosid" = lagoslakeid) ]
MI.ALL.Data[ , .N  , .( is.na(link_nhd_id)) ] 
...
MI.ALL.Data[MI.allsites, on = .(MonitoringLocationIdentifier = MonitoringLocationIdentifier) , ':=' ("latitude" = LatitudeMeasure, "longitude" = LongitudeMeasure, "agency" = agency_cd ) ]

setnames(MI.ALL.Data,
         old = c( "Year" , "DOY", "Date", "Depth", "Temperature", "DO",  "MonitoringLocationName", "MonitoringLocationIdentifier"),
         new = c( "year", "doy", "date", "depth_m", "temp_c",  "do_ppm",  "monitoringlocationname", "monitoringlocationidentifier" )) 

MI.ALL.Data[ , c("rn", "Latitude", "Longitude") := NULL , ]
MI.ALL.Data[ , date := as.IDate(date) , ]

MN_WI_profiles <- fread("data&scripts/data/output/MN_WI_profiles.csv")


MN_WI_MI_profiles <- rbindlist(list(MN_WI_profiles, MI.ALL.Data), fill = TRUE, use.names = TRUE)


fwrite(MN_WI_MI_profiles, file = "data&scripts/data/output/MN_WI_MI_profiles.csv") 


MN_WI_MI_profiles[ , .N , .(state, year) ]
MN_WI_MI_profiles[ , length(unique(profile_ident)) , .(state) ]

MN_WI_MI_profiles[ , hist(year) , .(state) ]
