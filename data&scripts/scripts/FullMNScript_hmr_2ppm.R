...
#' This read into r has issues-- seems to be squashing SUS/ suspect data codes into NA values. Could use fread:
...
MN.allsites %>% filter(row_number() %in% as.vector(problems(MN.allsites)[,"row"]))

#' This read into r also has issues-- seems to be squashing some varibale about watershed area... 
...
#' yes, but these are not DOWs, see 3 commands ago...
all.MN.DOW %>% 
  filter(OrganizationIdentifier != "MNPCA") %>% 
  group_by(DOW)%>%
  summarise(n = n()) #4,755 Lakes
...
# munging: round all depths to whole m ------------------------------------
