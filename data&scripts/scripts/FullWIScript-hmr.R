...
#Clean
rm(allWI.do, LakeDepthArea, Link, NTL, NTL.data, WI.allsites,
...
###Need to remove a couple measurements from ONE profile that didn't get filtered out, but are in metalimnion.

WI.DS.Hypo$obs<-1:nrow(WI.DS.Hypo)
  
dfX<-WI.DS.Hypo%>%
  filter(Year=="1982"&MonitoringLocationIdentifier=="WIDNR_WQX-1842400"&Depth<14)

WI.DS.Hypo<-WI.DS.Hypo%>%
  filter(!obs%in%dfX$obs)%>%
  select(!obs)

# #  Data Summary #25397 obs#HMR-this is down to 25376 after fixing one lake-year
# WI.DS.Hypo %>%
#   group_by(MonitoringLocationIdentifier)%>%
...
WI.do.tod.meta = read.csv("C:/Users/herantal/OneDrive - State of Minnesota - MN365/Documents/Coldwater_MGLP2020/Jacob/Wisconsin.Data/WI_Hyp_Search.csv",
                          fileEncoding = "latin1")%>%
  group_by(wibic)%>%
  filter(n == max(n))%>%
...
DS.WI$DOY_3<-(3-DS.WI$Intercept)/DS.WI$TOD #day of year when DO hits 2
